...
func (cm *ConsensusModule) runElectionTimer() {
	...
	for {
		...
		if cm.currentTerm >= 1 {
			cm.dlog("elapsed: %d, duration: %d", time.Since(cm.electionResetEvent), timeoutDuration)
		}
		if elapsed := time.Since(cm.electionResetEvent); elapsed >= timeoutDuration {
			cm.dlog("before election")
			cm.startElection()
			cm.dlog("before unlock")
			cm.mu.Unlock()
			return
		}
		cm.mu.Unlock()
...
func (cm *ConsensusModule) startElection() {
	...
	for _, peerId := range cm.peerIds {
		cm.dlog("before RequestVote goroutine")
		go func(peerId int) {
